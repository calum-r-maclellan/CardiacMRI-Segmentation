...
Vanilla models for medical image segmentation.
...
# wee function for returning the number of params 
...
class UNet(nn.Module):
    def __init__(self, in_channels, n_classes):
        ...
        self.Out = out_conv(64, n_classes)
    ...
    def forward(self, x):
        ...
        u4 = self.Up5(x5)                 # only dimensions doubled, Fms still 512 
        cat4 = torch.cat((x4,u4), dim=1)   # concat 512 with 512 to give 1024fms
        d4 = self.Up_conv4(cat4)           # 4th layer: 1024 -> 256
        ...
        net_out = self.Out(d1)              # Raw scores Output: 64 -> 1Fm. 

        return F.log_softmax(net_out, dim=1)  # log probs for F.nll_loss()
