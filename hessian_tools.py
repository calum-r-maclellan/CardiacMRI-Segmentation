"""
collection of useful pytorch functions for working with the hessian.

important for understanding the loss landscape, and the trajectory and 
minima selected by SGD.

In this first version the algorithm only track the TOP eigenvalue of the hessian.
Future versions will include the option to track the top K eigenvalues.

started: 30.1.21. 

updates: 
    - 2.2.21: fixed issue with .view and memory error.
     -> simple fix: added .contiguous() to tensor to maintain tensor alignment  (line 99)

@author: calmac
adapted from sgd.stability fork on github.
"""
...
def Hv_batch(net, criterion, batch_x, batch_y, v):
    ...
    idx, res = 0, 0
    for grad_i in grads:
        ng = torch.numel(grad_i) # returns total number of gradients  
        v_i = v[idx:idx+ng].cuda()
        res += torch.dot(v_i, grad_i.view(-1))
        idx += ng
    Hv = autograd.grad(res, net.parameters())
    Hv = [t.contiguous().data.cpu().view(-1) for t in Hv]
    Hv = torch.cat(Hv)
    return Hv
